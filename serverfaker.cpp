...
/*
	...
	printf("\n");
    // main loop
	do {} while (net_CheckForDataOnSocket (socket)!=1);
		net_receiveData (socket); //The received data is in ReceiveBuffer!!

	printf(ReceiveBuffer);

	net_addClientToAddressBook (1);
	net_sendReply ("sendReply Answer from server\n\0", socket);
	net_sendData ("sendData Answer from server!\n\0", socket, 1);
	} while (1==1);
...
*/
    ...
    net_init ();
    socket = net_connect ("192.168.0.200", 1370, 0); //IP Address, port, and receiving client ID (which is always 0 if we are a client sending data to the server!!)
    if (socket==-1)
	exit (1);
	...
	do {
		...
		if (dwTime % 4000 == 0)
		{
			net_sendDataToServer ( "Z 212.17.120.182 Gameserver_Sync-Test 128 2 1", socket); //Data to be sent, socket, and connection number
			printf("%i \n", dwTime);
		}

	} while (1==1);
	
//	net_sendDataToServer ( "Z 192.168.0.200 Playspoon_#1 8 0 1", socket); //Data to be sent, socket, and connection number
//	net_sendDataToServer ( "Z 192.168.0.201 Playspoon_#2 8 0 1", socket); //Data to be sent, socket, and connection number
//	net_sendDataToServer ( "Z 192.168.0.203 Playspoon_#3 8 0 1", socket); //Data to be sent, socket, and connection number
//	net_sendDataToServer ( "Z 193.170.255.64 [FHE]BroadbandWar 32 0 1", socket); //Data to be sent, socket, and connection number
//	net_sendDataToServer ( "Z 212.120.182.233 Chello.at 64 0 1", socket); //Data to be sent, socket, and connection number
//	net_sendDataToServer ( "Z 192.168.0.205 Laptop 4 0 1", socket); //Data to be sent, socket, and connection number
//	net_sendDataToServer ( "Z 192.168.0.204 Marianne_Rechner 16 0 1", socket); //Data to be sent, socket, and connection number

	net_sendDataToServer ( "Y", socket); //Data to be sent, socket, and connection number

	printf("Data sent, waiting for answer...\n\n");
